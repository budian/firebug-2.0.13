...
Xpath.getElementXPath = function(element)
{
    ...
    if(element && element.localName){
        xpath = '//' + element.localName + '[';
     }
    if (element && element.id){
        index ++;
        conditions[index] = '@id="' + element.id + '"';
    }
    if(element && element.name){
        index ++;
        conditions[index] = '@name="' + element.name + '"';
    }
    if (element && element.classList){
        /*if(element.classList.length==1){
            index ++;
            conditions[index] = '@class="' + element.classList[0] + '"';
        }*/
        for (var i=0, len=element.classList.length; i<len; ++i){
            index ++
            conditions[index] = 'contains(@class,"' + element.classList[i] + '")';
        }
    }
    if(index!=-1){
        for(var i=0;i<=index; i++){
            if(i==index){
                xpath = xpath + conditions[i];
            }else{
                xpath = xpath + conditions[i] + " and ";
            }
        }
        xpath = xpath +  ']';
        /*if(element.localName=="frame" || element.localName=="iframe"){
            var xpath2 = Xpath.getElementTreeXPath(element);
            xpath = xpath + "[" + "]";
        }*/
        return xpath;
    }else{
